...
class Question extends Model
{
    ...
    public function getdatatable_all(){
        $requestData = $_REQUEST;
        ...
        $resultArr = $query->skip($requestData['start'])
                        ->take($requestData['length'])
                        ->select('question.id', 'question.question', 'question.is_active', 'question.quiz_id', 'quiz.name', 'quiz_type.name as quiz_type', 'quiz_category.name as quiz_category')
                        ->get();
        $data = array();
        $i = 0;
        ...
        foreach ($resultArr as $row) {
            ...
            $nestedData = array();
            $nestedData[] = $i;
            $nestedData[] = $row['quiz_type'];
            $nestedData[] = $row['quiz_category'];
            $nestedData[] = $row['name'];
            $nestedData[] = $row['question'];
            $nestedData[] = $status;
            $nestedData[] = $actionhtml;
